...
exports.update = function(request, response) {    
    try {
        ...
        console.debug("Update permission request: " + uid);
...
function checkPermissionAndUpdate (request, response) {
    db.executeQuery(`SELECT  
        ...
        function(err, rows) {
            ...
            if (isOwner) {
                return deletePermissions(request, response);
            }
...
function deletePermissions (request, response) {
    db.executeQuery('DELETE FROM FilesRolesPermission WHERE fileId = ?', uid, function(err, result) {
        if (err) {
            console.error(err);
            responseHelper.triggeredException(response, new FileException(500, "Internal error (cannot update file permission informations)"));
...
function insertPermissions (request, response) {
    ...
    responseHelper.reply(response, "Permissions updated: " + uid);
}